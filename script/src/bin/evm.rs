...
use pq_bitcoin_lib::{
    PublicValuesStruct, compute_btc_migration_message, ml_dsa_level_name,
    public_key_to_btc_address, validate_pq_pubkey,
};
use rand::TryRngCore;
use rand::rngs::OsRng;
...
fn main() {
    ...
    let (sig, pub_key, address) = sign(&secp, seckey, &pq_public_key_bytes).unwrap();
